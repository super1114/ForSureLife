...
class OutstandingTestingPageClass extends React.Component<Props, any> {
  yes = () => {
    const { history, updateHealthQuestion } = this.props;
    navigate(history, navRoutes.Health.SeriousComplications.path);
    updateHealthQuestion({
      ...
      <div>
        <CardWrapper>
          <div className="d-flex justify-between flex-column h-100">
            <QuestionDisplay
              questionTitle={
                QuestionList.HealthQuestions.OutstandingTesting.Title
              }
              questionText={
                QuestionList.HealthQuestions.OutstandingTesting.Text
              }
            />
            <div>
              <YesNoAnswer
              ...
              />
              <NavigationButtons
                progressPercent={flow == LeadFlow.E ? 65 : 54}
                hideProgress
                QueCounter={true}
                hideNext
                activeCounter={QuestionList.HealthQuestions.OutstandingTesting.QuestionNumber}
                navigate={() =>
...
export const OutstandingTestingPage = connect(
  mapStateToProps,
  mapDispatchToProps
)(OutstandingTestingPageClass);
