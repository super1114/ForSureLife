...
class CirrhosisDiagnosisPageClass extends React.Component<Props, any> {
  yes = () => {
    const { history, updateHealthQuestion } = this.props;
    navigate(history, navRoutes.Health.CirrhosisTimeline.path);
    updateHealthQuestion({
  ...
  no = () => {
    const { history, updateHealthQuestion } = this.props;
    navigate(history, navRoutes.Health.KidneyDisease.path);
    updateHealthQuestion({
      applicationQuestion: ApplicationHealthQuestions.HepatitisC,
      healthAnswer: false,
    });
      ...
      <div>
        <CardWrapper>
          <div className="d-flex justify-between flex-column h-100">
            <QuestionDisplay
              questionTitle={
                QuestionList.HealthQuestions.CirrhosisDiagnosis.Title
              }
              questionText={
                QuestionList.HealthQuestions.CirrhosisDiagnosis.Text
              }
            />
            <div>
              <YesNoAnswer
              ...
              />
              <NavigationButtons
                progressPercent={flow == LeadFlow.E ? 46 : 12}
                hideProgress
                QueCounter={true}
                hideNext
                activeCounter={QuestionList.HealthQuestions.CirrhosisDiagnosis.QuestionNumber}
                navigate={() =>
...
export const CirrhosisDiagnosisPage = connect(
  mapStateToProps,
  mapDispatchToProps
)(CirrhosisDiagnosisPageClass);
